...
int _open(const char *path, int flags, mode_t mode) {
  return _syscall_((intptr_t)path, (intptr_t)flags, (intptr_t)mode);
}
...
void *_sbrk(intptr_t increment) {
  ...
  p_brk += increment;
  if(!_syscall_((intptr_t)SYS_brk, (intptr_t)p_brk, 0, 0)) {
    ret = (void *)old_brk;
  }
  else {
    ret = (void *)-1;
...
int _read(int fd, void *buf, size_t count) {
  return _syscall_((intptr_t)SYS_read, (intptr_t)buf, (intptr_t)count);
}

int _close(int fd) {
  return _syscall_((intptr_t)SYS_close, (intptr_t)fd);
}

off_t _lseek(int fd, off_t offset, int whence) {
  return _syscall((intptr_t)SYS_lseek, (intptr_t)offset, (intptr_t)whence); 
}
