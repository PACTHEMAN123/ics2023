...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->pc + 4; s->dnpc = (src1 + imm)&(~1); if(rd == 0 && imm == 0)output_ftrace(s->pc, s->dnpc, 0); else output_ftrace(s->pc, s->dnpc, 1));
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal	   , J, R(rd) = s->pc + 4; s->dnpc = s->pc + imm; output_ftrace(s->pc, s->dnpc, 1));
