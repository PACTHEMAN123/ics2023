...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->pc + 4; s->dnpc = (src1 + imm)&(~1); output_ftrace(s->pc, s->dnpc));
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal	   , J, R(rd) = s->pc + 4; s->dnpc = s->pc + imm; output_ftrace(s->pc, s->dnpc));
...
void output_ftrace(vaddr_t pc, vaddr_t dnpc) {
  for(int i = 0; i < findex; i++) {
    if((dnpc >= func[i].value) && (dnpc < func[i].value + func[i].size)){
      Log("%#8x call [%s@%#8x]", pc, func[i].name, dnpc); 
    } 
  }
}
